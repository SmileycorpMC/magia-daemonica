...
public class SummoningCircleRenderer implements RitualRenderer<SummoningCircle> {
    ...
    public void render(SummoningCircle ritual, double x, double y, double z, int width, int height, float partialTicks) {
        ...
        boolean hasLighting = ritual.getTicksActive() <= 100;
        //setup lighting
        if (hasLighting) {
        ...
        buffer.pos(0, 0.01, 0).tex(0, 0).color(r, g, b, 1).normal(0, 1, 0).endVertex();
        buffer.pos(0, 0.01, height).tex(0, 1).color(r, g, b, 1).normal(0, 1, 0).endVertex();
        buffer.pos(width, 0.01, height).tex(1, 1).color(r, g, b, 1).normal(0, 1, 0).endVertex();
        buffer.pos(width, 0.01, 0).tex(1, 0).color(r, g, b, 1).normal(0, 1, 0).endVertex();
        tessellator.draw();
        if (hasLighting) {
