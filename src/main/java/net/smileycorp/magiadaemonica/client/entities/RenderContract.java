...
public class RenderContract extends Render<EntityContract> {
    ...
    public void doRender(EntityContract entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        //contract
        buffer.pos(0, 0.5 - height, 0).tex(0, height * 2).endVertex();
        buffer.pos(0.5, 0.5 - height, 0).tex(1, height * 2).endVertex();
        ...
        if (entity.ticksExisted < 40) {
            ...
            TextureAtlasSprite sprite = Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite("minecraft:blocks/fire_layer_1");
            mc.getTextureManager().bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE);
            GlStateManager.pushMatrix();
            float v = sprite.getMinV() + (sprite.getMaxV() - sprite.getMinV()) * 0.75f;
            GlStateManager.translate(dir.x * 0.025, 0, dir.z * 0.025);
            buffer.begin(7, DefaultVertexFormats.POSITION_TEX);
            buffer.pos(0, 0.375 - height, 0).tex(sprite.getMinU(), sprite.getMaxV()).endVertex();
            buffer.pos(0.5, 0.375 - height, 0).tex(sprite.getMaxU(), sprite.getMaxV()).endVertex();
